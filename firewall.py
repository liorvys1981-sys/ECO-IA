...
def _is_valid_ip(ip: str) -> bool:
    ...
    if any(int(m.group(i)) > 255 for i in (1, 2, 3, 4)):
        return False
    if m.group(6) is not None and int(m.group(6)) > 32:
        return False
    return True
...
class FirewallManager:
    ...
    def block_ip(self, ip: str, reason: str = "") -> dict[str, Any]:
        ...
        if rec["success"] and ip not in self._blocked_ips:
            self._blocked_ips.append(ip)
        self._rule_log.append(rec)
        return rec
    ...
    def unblock_ip(self, ip: str) -> dict[str, Any]:
        ...
        rec = {"action": "unblock", "ip": ip, "success": r["returncode"] == 0,
               "timestamp": datetime.utcnow().isoformat()}
        if rec["success"] and ip in self._blocked_ips:
    ...
    def get_status(self):
        ...
        return {
            "ufw_output": r["stdout"], "blocked_ips": list(
                self._blocked_ips), "total_rules": len(
                self._rule_log)}
