...
class ResourceOptimizer:
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "psutil not installed; returning simulated metrics.")
    ...
    def _simulate_metrics(self) -> dict[str, Any]:
        ...
        return {
            "cpu_percent": round(rng.uniform(10, 60), 1),
            "ram_percent": round(rng.uniform(30, 70), 1),
            "ram_used_gb": round(rng.uniform(1, 6), 2),
            "ram_total_gb": 8.0,
            "disk_percent": round(rng.uniform(20, 50), 1),
            "disk_used_gb": round(rng.uniform(20, 80), 2),
            "disk_total_gb": 160.0,
            "net_bytes_sent_mb": round(rng.uniform(100, 5000), 2),
            "net_bytes_recv_mb": round(rng.uniform(100, 5000), 2),
        }
    ...
    def analyse(self) -> dict[str, Any]:
        ...
        if metrics["cpu_percent"] > self.cpu_threshold:
            recommendations.append(
                f"CPU usage is high ({
                    metrics['cpu_percent']}%). Consider scaling up or optimising workloads.")
        if metrics["ram_percent"] > self.ram_threshold:
            recommendations.append(
                f"RAM usage is high ({
                    metrics['ram_percent']}%). Consider adding swap or scaling RAM.")
        if metrics["disk_percent"] > 80:
            recommendations.append(
                f"Disk usage is high ({
                    metrics['disk_percent']}%). Run cleanup or expand storage.")
